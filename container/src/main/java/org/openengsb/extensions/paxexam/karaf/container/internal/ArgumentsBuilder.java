...
class ArgumentsBuilder {
    ...
    /**
     * Controls if one of the options set a Args Option manually. Otherwise, defaultArguments will include a --noArgs
     * flag to prevent unintentional runner.args files being picked up by paxrunner.
     */
        ...
        throws IOException
    {
        ...
        // add(m_paxrunneArguments, extractArguments(system.getOptions(CustomFrameworkOption.class)));
    ...
    /**
     * Wrap provision options that are not already scanner provision bundles with a
     * {@link org.ops4j.pax.exam.options.extra.BundleScannerProvisionOption} in order to force update.
     * 
     ...
     * @param excludeDefaultRepositoriesOptions if array not empty the default list of maven repos should be excluded
     * 
     * @return converted Pax Runner argument
     */
    ...
    private List<String> extractArguments(RawPaxRunnerOptionOption[] paxrunnerOptions)
    {
        ...
        if (paxrunnerOptions.length > 0 || excludeDefaultRepositories) {
            for (RawPaxRunnerOptionOption paxrunnerOption : paxrunnerOptions) {

                args.add(paxrunnerOption.getOption().trim());
            }
        }
        return args;
    }
